// i18n Localization Dictionary
const CA_I18N = {
    th: {
        tooltipAddWayspot: "เพิ่ม Wayspot",
        tooltipEditPosition: "แก้ไขตำแหน่ง",
        tooltipWayspotList: "รายการ Wayspot",
        tooltipDrawMode: "วาดเขตเลือก Wayspot",
        tooltipUndo: "ย้อนกลับ (Cmd+Z/Ctrl+Z)",
        tooltipSettings: "การตั้งค่า",
        tooltipMyLocation: "ตำแหน่งของฉัน",
        tooltipCapture: "📸 บันทึกภาพแผนที่",
        tooltip3DSim: "🎮 จำลองตำแหน่ง 3D",
        tooltipMenu: "☰ เมนู",
        tooltipCopy: "คัดลอกรหัส",
        addWayspotTitle: "➕ เพิ่ม Wayspot",
        wayspotTypeLabel: "ประเภท Wayspot",
        optPokestop: "PokéStop",
        optGym: "Gym",
        optCaPokestop: "CA PokéStop",
        optCaGym: "CA Gym",
        optClWayspot: "CL Wayspot 🍊",
        optClGymWayspot: "CL Gym Wayspot 🟢",
        optPowerSpot: "พาวเวอร์ สป๊อต",
        optNoneWayspot: "เสาที่ไม่ได้ใช้ในเกม",
        nameLabel: "ชื่อ",
        namePlaceholder: "ศาลพระภูมิ...",
        imgUrlLabel: "URL รูปภาพ (ไม่บังคับ)",
        latLabel: "ละติจูด (Lat)",
        lngLabel: "ลองจิจูด (Lng)",
        radiusLabel: "รัศมี (เมตร)",
        btnAddByCoords: "เพิ่มด้วยพิกัด",
        addByMapHint: "หรือคลิก 2 ครั้งในตำแหน่งทีต้องการเพื่อเพิ่มหมุด",
        allWayspotsTitle: "รายการทั้งหมด",
        settingsTitle: "การตั้งค่า",
        mapLayerLabel: "การแสดงผลแผนที่",
        optGmapStreet: "Google Maps (ถนน)",
        optGmapSat: "Google Maps (ดาวเทียม)",
        optOsm: "OpenStreetMap",
        optCartoDark: "CartoDB DarkMatter",
        optBing: "Bing Maps (ดาวเทียม)",
        optMapbox: "Pokemon GO Map",
        optGmapStyled: "Google Maps (Pokemon GO Style)",
        gmapApiKeyLabel: "Google Maps API Key",
        applyLabel: "ใช้งาน",
        mapboxTokenLabel: "Mapbox Access Token",
        mapboxStyleLabel: "Mapbox Style URL",
        mapFeatureTitle: "ฟีเจอร์แผนที่",
        darkMode: "🌙 โหมดกลางคืน",
        s2Grid: "🌐 แสดง S2 Cells (L14, L17)",
        draggable: "✋ อนุญาตให้ลากหมุด",
        iconSizeLabel: "ปรับขนาดรูป Wayspot (ขนาดเป็น px)",
        showRadiusLabel: "แสดงวงรัศมีของ Wayspot",
        showExclusionZoneLabel: "🚫 แสดงพื้นที่ทับซ้อน (45ม.)",
        visibilityTitle: "การแสดงผล Wayspot",
        forceRadiusTitle: "บังคับเปลี่ยนรัศมี Wayspot",
        radiusPlaceholder: "ระบุระยะ (เช่น 20 หรือ 45)",
        forceRadiusBtn: "เปลี่ยนระยะรัศมีทั้งหมด",
        cloudSyncTitle: "Cloud Sync (Google Drive)",
        saveToDrive: "☁️ อัปโหลดไปยัง Google Drive",
        loadFromDrive: "☁️ ดาวน์โหลดจาก Google Drive",
        localDataTitle: "การจัดการแฟ้มข้อมูล (Local)",
        importKML: "📥 นำเข้า KML",
        exportKML: "📤 ส่งออก KML",
        importJSON: "📥 นำเข้า JSON",
        exportJSON: "📤 ส่งออก JSON",
        clearAll: "🗑️ ล้างข้อมูล Wayspot ทั้งหมด",
        langTitle: "ภาษา (Language)",
        customColorsTitle: "กำหนดสี Wayspot",
        btnResetColors: "คืนค่าสีดั่งเดิม",
        accMapThemes: "🗺️ แผนที่และธีม",
        accWayspotStyles: "🎨 การแสดงผล Wayspot",
        accControlTools: "🛠️ เครื่องมือควบคุม",
        accSyncData: "☁️ การจัดการข้อมูล Wayspot",
        accAboutApp: "🌐 เลือกภาษา",
        credits: "จัดทำโดย CA: Community Ambassador Thailand",
        btnDone: "เสร็จสิ้น",
        searchPlaceholder: "ค้นหาชื่อ Wayspot...",
        accProjects: "📂 จัดการโปรเจค",
        btnNewProject: "➕ สร้างโครงการใหม่",
        defaultProjectName: "โปรเจกต์เริ่มต้น",
        projectNameLabel: "ชื่อโครงการ",
        newProjectPrompt: "กรุณาระบุชื่อโครงการใหม่:",
        renameProjectPrompt: "แก้ไขชื่อโครงการ:",
        deleteProjectConfirm: "คุณต้องการลบโครงการ '{name}' ใช่หรือไม่? (ข้อมูลในโครงการนี้จะถูกลบทั้งหมด)",
        duplicateSuffix: " (สำเนา)",
        backupOptionsTitle: "เลือกการสำรองข้อมูล",
        backupActiveOnly: "สำรองเฉพาะโปรเจกต์ปัจจุบัน",
        backupAllProjects: "สำรองทุกโปรเจกต์",
        editTitle: "แก้ไขข้อมูล",
        editTypeLabel: "ประเภท",
        btnCancel: "ยกเลิก",
        btnSave: "บันทึก",
        btnNext: "ถัดไป",
        btnBack: "ย้อนกลับ",
        btnUnderstand: "เข้าใจแล้ว",
        menuTutorial: "📘 การใช้งานเบื้องต้น",
        tutorialTitle: "การใช้งานเบื้องต้น",
        tutorialSlide1Title: "📍 การเพิ่มตำแหน่ง",
        tutorialSlide1Content: "คลิก 2 ครั้งในตำแหน่งทีต้องการเพื่อเพิ่มหมุด หรือกรอกพิกัดโดยตรงเพื่อความแม่นยำ!",
        tutorialSlide2Title: "✏️ การแก้ไขและย้อนกลับ",
        tutorialSlide2Content: "สลับเข้าโหมดแก้ไขเพื่อลากย้ายหมุด และใช้ปุ่มย้อนกลับ (↩️) หรือ Cmd+Z/Ctrl+Z เพื่อคืนค่ากรณีทำผิดพลาด",
        tutorialSlide3Title: "🌐 เครื่องมือวางแผน",
        tutorialSlide3Content: "เปิด S2 Cells (L14/L17) และวงรัศมี 45 เมตร เพื่อตรวจสอบการเกิดของเสาและการทับซ้อน",
        tutorialSlide4Title: "🎨 ปรับแต่งหน้าตา",
        tutorialSlide4Content: "สลับเป็น Dark Mode เพื่อถนอมสายตา และเลือกชั้นข้อมูลแผนที่ (Google/OSM) ได้ตามต้องการ",
        tutorialSlide5Title: "☁️ จัดการข้อมูลและคลาวด์",
        tutorialSlide5Content: "ซิงค์ข้อมูลขึ้น Google Drive หรือนำเข้า/ส่งออกไฟล์ JSON เพื่อเก็บข้อมูลของคุณให้ปลอดภัย",
        tutorialSlide6Title: "📥 ดึงข้อมูลจาก Wayfarer",
        tutorialSlide6Content: "ดาวน์โหลดส่วนเสริมสคริปต์ CA Exporter ในแถบตั้งค่า เพื่อดึงข้อมูล Wayspot และสถานที่เข้าแผนที่ได้ด้วยคลิกเดียว",
        tutorialSlide7Title: "🤝 ร่วมกันทำงานแบบเรียลไทม์",
        tutorialSlide7Content: "สร้างรหัสเซิร์ฟเวอร์แบบชั่วคราวและส่งให้ทีมของคุณ เพื่อช่วยกันอัปเดตและปักหมุดพร้อมๆ กันบนแผนที่ในเวลาเดียวกัน",
        welcomeTitle: "สวัสดีครับ 👋",
        welcomeText: "<b>CA Wayspot Tools</b> โฉมใหม่!<br><br>จำลอง กะระยะ และวางแผนการตั้ง CA Wayspot ได้ง่ายขึ้น รองรับการวาด S2 Cells และไฟล์ KML จาก Google My Maps<br><br>ลองคลิกบนแผนที่เพื่อปักหมุดแรกได้เลยครับ",
        btnStart: "เริ่มต้นใช้งาน",
        noDataToSave: "ไม่มีข้อมูลที่จะบันทึก",
        saveSuccess: "บันทึกลง Google Drive สำเร็จ! ☁️",
        saveFailed: "บันทึกไม่สำเร็จ Error: ",
        backupNotFound: "ไม่พบไฟล์แบ็กอัพใน Google Drive ของคุณ",
        loadSuccess: "โหลดข้อมูลสำเร็จ! ☁️",
        loadFailed: "กู้คืนไม่สำเร็จ: ",
        importSuccessCount: "นำเข้าสำเร็จ {count} จุด\n(ข้ามข้อมูลซ้ำ {dup} จุด)",
        noLocationFound: "ไม่พบข้อมูลตำแหน่ง",
        invalidLatLong: "กรุณากรอกพิกัด Latitude และ Longitude ให้ครบและถูกต้อง",
        deleteConfirmMark: "ลบ Wayspot นี้ออก?",
        invalidRadius: "กรุณากรอกตัวเลขระยะรัศมีให้ถูกต้อง",
        confirmRadiusChange: "ต้องการเปลี่ยนรัศมีของตำแหน่งทั้งหมดเป็น {radius} เมตร ใช่หรือไม่?",
        radiusChangeSuccess: "เปลี่ยนระยะรัศมีทั้งหมด {count} จุดสำเร็จ!",
        clearAllConfirm: "คุณต้องการลบข้อมูลทั้งหมดจริงๆ หรือไม่?",
        unnamedAlert: "ไม่มีชื่อ",
        countLabel: "จำนวน:",
        infoCoords: "พิกัด:",
        btnEdit: "แก้ไข",
        btnLock: "🔒 ล็อก",
        btnUnlock: "🔓 ปลดล็อก",
        btnHideSpot: "👁 ซ่อน",
        btnShowSpot: "🙈 แสดง",
        btnDelete: "ลบ",
        editWayspot: "แก้ไข Wayspot",
        errorLocation: "ไม่สามารถเข้าถึงตำแหน่งของคุณได้ โปรดลองอีกครั้งในภายหลัง",
        importError: "เกิดข้อผิดพลาดในการโหลดไฟล์ข้อมูล พยายามตรวจสอบรูปแบบไฟล์อีกครั้ง",
        themeLabel: "ธีมของ UI",
        themeDefault: "Theme พื้นฐาน",
        themePokemon: "Pokemon GO Theme",
        themeIos26: "ธีม Liquid Glass",
        themeDark: "Dark Theme",
        invalidName: "กรุณาระบุชื่อ Wayspot",
        invalidImageUrl: "รูปแบบ URL รูปภาพไม่ถูกต้อง",
        accCollab: "🤝 ร่วมกันทำงานแบบเรียลไทม์",
        collabHostBtn: "สร้างห้อง",
        collabJoinBtn: "เข้าร่วม",
        collabIdPlaceholder: "ใส่รหัสห้อง...",
        collabStatusPrefix: "สถานะ: ",
        collabDisconnected: "ไม่ได้เชื่อมต่อ",
        collabConnectedHost: "ห้อง {id}",
        collabConnectedClient: "ห้อง {id}",
        collabDisconnectBtn: "ยกเลิกการเชื่อมต่อ",
        collabSyncBtn: "ดึงข้อมูลล่าสุด",
        collabSyncing: "กำลังซิงค์... ({count} จุด)",
        collabWait: "กำลังรอเพื่อนเข้าร่วมรหัส: {id}",
        collabConnectedHostRes: "เชื่อมต่อกับเพื่อนสำเร็จ!",
        collabConnectedClientRes: "เชื่อมต่อห้อง {id} สำเร็จ!",
        collabConnectedClient: "เชื่อมต่อห้อง {id} สำเร็จ!",
        collabError: "เชื่อมต่อไม่สำเร็จ: ",
        collabDisconnectBtn: "ยกเลิกการเชื่อมต่อ",
        importWayfarerTitle: "นำเข้าข้อมูล Wayspot จากเว็บ Niantic Wayfarer",
        btnDownloadScript: "ดาวน์โหลด Script",
        btnInstallInstructions: "วิธีติดตั้ง",
        installInstructionsList: "1. กดปุ่ม \"ดาวน์โหลด Script\"<br>2. ติดตั้ง Script ผ่านทาง tampermonkey หรือ Userscripts<br>3. ล็อกอินเข้าเว็บ Niantic Wayfarer และไปที่หน้า Mapview<br>4. กดปุ่ม Export CA ระบบจะดาวน์โหลดไฟล์ที่ชื่อ CA_Wayfarer_500m_XXXX.json<br>5. นำไฟล์ CA_Wayfarer_500m_XXXX.json ผ่านช่องทาง \"นำเข้า JSON\"<br>6. เอนจอย 🥰",
        dragDropText: "ลากไฟล์มาวางที่นี่เพื่อนำเข้าข้อมูล",
        accLivePoi: "📡 POI จาก Wayfarer",
        livePoiPowerSpotLabel: "Power Spot",
        livePoiTokenSectionTitle: "Campfire Session Token",
        livePoiTokenPlaceholder: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
        btnClear: "ล้าง",
        livePoiLoading: "กำลังโหลด POI...",
        livePoiZoomInHint: "ซูมเข้ามากกว่านี้เพื่อดู POI",
        livePoiTokenExpired: "Token หมดอายุหรือไม่ถูกต้อง กรุณาขอ Token ใหม่",
        livePoiFetchError: "โหลดข้อมูล POI ไม่สำเร็จ",
        livePoiNetworkError: "เชื่อมต่อไม่สำเร็จ — ตรวจสอบอินเทอร์เน็ต",
        livePoiReadOnlyNote: "ข้อมูลจากทาง Wayfarer (ดูอย่างเดียว)",

        livePoiTokenCardTitle: "Campfire API Token",
        livePoiTokenNotConfigured: "ยังไม่ได้ตั้งค่า — ต้องตั้งค่าก่อนถึงจะดู Power Spot ได้",
        livePoiTokenExpiredShort: "⚠ Token หมดอายุแล้ว — กรุณาอัพเดต token ใหม่",
        livePoiTokenReady: "พร้อมใช้งาน",
        livePoiTokenUpdatedAt: "อัพเดต",
        livePoiTokenBtnSet: "ตั้งค่า Token",
        livePoiTokenBtnUpdate: "อัพเดต Token",
        livePoiHowToComputer: "💻 วิธีง่าย — คอมพิวเตอร์",
        livePoiHowToComputerStep1: "ลากปุ่มนี้ไปวางบน Bookmark bar:",
        livePoiHowToComputerStep2: "เปิด campfire.nianticlabs.com แล้ว login ด้วยบัญชีของตัวเอง",
        livePoiHowToComputerStep3: "กดปุ่มที่ลากไว้ใน Bookmark bar",
        livePoiHowToComputerStep4: "กลับมาที่นี่ กดปุ่ม \"วางจาก Clipboard\" ด้านล่าง",
        livePoiHowToMobile: "📱 วิธีง่าย — iPhone/iPad (Safari)",
        livePoiHowToMobileStep1: "กดลิงก์นี้ (ครั้งเดียว) เพื่อติดตั้ง Shortcut:",
        livePoiHowToMobileStep1b: "(หรือสแกน QR code)",
        livePoiHowToMobileShortcutLink: "เพิ่ม Shortcut \"Campfire Token\"",
        livePoiHowToMobileStep2: "เปิด campfire.nianticlabs.com ใน Safari แล้ว login ด้วยบัญชีของตัวเอง",
        livePoiHowToMobileStep3: "กด Share → เลือก Shortcut \"Campfire Token\" (ครั้งแรกต้องกด Allow)",
        livePoiHowToMobileStep4: "กลับมาที่นี่ กดปุ่ม \"วางจาก Clipboard\" ด้านล่าง",
        livePoiPasteClipboard: "📋 วางจาก Clipboard",
        livePoiBearerLabel: "Bearer Token *",
        livePoiCsrfLabel: "X-Csrf-Token (ไม่บังคับ)",
        livePoiHintText: "💡 ถ้า 2 วิธีด้านบนใช้ไม่ได้ — เปิด campfire.nianticlabs.com → DevTools → Network → คลิก request ไปที่ niantic-social-api.nianticlabs.com/graphql → ดู Request Headers → Authorization (ตัด \"Bearer \" ออก)",
        livePoiPrivacyNote: "🔒 ค่า Campfire API Token จะถูกเก็บในเครื่องนี้เท่านั้น จะไม่ถูกอัปโหลด หรือส่งไปให้บุคคลใดบุคคลหนึ่งเด็ดขาด",
        hideItemTooltip: "ซ่อน/แสดงจุดนี้",
        hideAllTooltip: "ซ่อน/แสดงทั้งหมดในหมวดนี้",
        selectAllTooltip: "เลือกทั้งหมด/ไม่เลือกทั้งหมดในหมวดนี้",
        livePoiFromWayfarerSuffix: "จาก Wayfarer",
        drawModeHint: "คลิกบนแผนที่เพื่อวาดเขต — ดับเบิ้ลคลิกเพื่อจบ",
        drawSelectionTitle: "พื้นที่ที่เลือก",
        drawNoSelection: "ไม่มี Wayspot อยู่ในเขตที่วาด",
        btnClearSelection: "ยกเลิกการเลือก",
        btnDeleteSelected: "ลบที่เลือก",
        btnExportSelectedJson: "📄 Export JSON",
        btnExportSelectedKml: "📤 Export KML",
        confirmDeleteSelected: "ยืนยันลบ {count} จุดที่เลือก?\n(Wayspot ของคุณจะถูกลบจริง ส่วน POI จาก Wayfarer จะถูกซ่อนออกจากการแสดงผล)",
        livePoiPasteError: "อ่าน Clipboard ไม่ได้ — ลองก็อปแล้ววางเองในช่อง Bearer Token ด้านล่าง",
        livePoiBearerRequired: "กรุณาใส่ Bearer Token",
        livePoiSaving: "กำลังตรวจสอบ...",
        livePoiTokenInvalid: "Token ใช้งานไม่ได้ — ตรวจสอบว่าคัดลอกมาถูกต้องและยังไม่หมดอายุ",
        wayfarerImportTitle: "📡 พบ Wayspot จาก Wayfarer",
        wayfarerImportDesc: "ไฟล์นี้มี Wayspot ที่มีเครื่องหมายว่ามาจาก Wayfarer อยู่ {count} จุด ต้องการนำเข้าอย่างไร?",
        wayfarerImportAll: "นำเข้าข้อมูลทั้งหมด",
        wayfarerImportUserOnly: "นำเข้าเฉพาะ Wayspot ที่เพิ่มเอง",
        wayfarerImportCancelled: "ยกเลิกการนำเข้าข้อมูลแล้ว"
    },
    en: {
        tooltipAddWayspot: "Add Wayspot",
        tooltipEditPosition: "Edit Position",
        tooltipWayspotList: "Wayspot List",
        tooltipDrawMode: "Draw area to select Wayspots",
        tooltipUndo: "Undo (Cmd+Z/Ctrl+Z)",
        tooltipSettings: "Settings",
        tooltipMyLocation: "My Location",
        tooltipCapture: "📸 Capture Map",
        tooltip3DSim: "🎮 3D Simulation",
        tooltipMenu: "☰ Menu",
        tooltipCopy: "Copy ID",
        addWayspotTitle: "➕ Add Wayspot",
        wayspotTypeLabel: "Wayspot Type",
        optPokestop: "PokéStop",
        optGym: "Gym",
        optCaPokestop: "CA PokéStop",
        optCaGym: "CA Gym",
        optClWayspot: "CL Wayspot 🍊",
        optClGymWayspot: "CL Gym Wayspot 🟢",
        optPowerSpot: "Power Spot",
        optNoneWayspot: "None (Unused in game)",
        nameLabel: "Name",
        namePlaceholder: "Shrine...",
        imgUrlLabel: "Image URL (Optional)",
        latLabel: "Latitude",
        lngLabel: "Longitude",
        radiusLabel: "Radius (meters)",
        btnAddByCoords: "Add by Coordinates",
        addByMapHint: "Or double click on map to add Wayspot",
        allWayspotsTitle: "All Wayspots",
        settingsTitle: "Settings",
        mapLayerLabel: "Map Layer",
        optGmapStreet: "Google Maps (Street)",
        optGmapSat: "Google Maps (Satellite)",
        optOsm: "OpenStreetMap",
        optCartoDark: "CartoDB DarkMatter",
        optBing: "Bing Maps (Satellite)",
        optMapbox: "Pokemon GO Map",
        optGmapStyled: "Google Maps (Pokemon GO Style)",
        gmapApiKeyLabel: "Google Maps API Key",
        applyLabel: "Apply",
        mapboxTokenLabel: "Mapbox Access Token",
        mapboxStyleLabel: "Mapbox Style URL",
        mapFeatureTitle: "Map Features",
        darkMode: "🌙 Dark Mode",
        s2Grid: "🌐 Show S2 Cells (L14, L17)",
        draggable: "✋ Allow Draggable Wayspot",
        iconSizeLabel: "Adjust Wayspot Icon Size (px)",
        showRadiusLabel: "Show Wayspot Radius",
        showExclusionZoneLabel: "🚫 Show Exclusion Zone (45m)",
        visibilityTitle: "Wayspot Visibility",
        forceRadiusTitle: "Force Change Radius",
        radiusPlaceholder: "Enter distance (e.g., 20 or 45)",
        forceRadiusBtn: "Change All Radiuses",
        cloudSyncTitle: "Cloud Sync (Google Drive)",
        saveToDrive: "☁️ Upload to Google Drive",
        loadFromDrive: "☁️ Download from Google Drive",
        localDataTitle: "Data Management (Local)",
        importKML: "📥 Import KML",
        exportKML: "📤 Export KML",
        importJSON: "📥 Import JSON",
        exportJSON: "📤 Export JSON",
        clearAll: "🗑️ Clear All Wayspot Data",
        langTitle: "Language",
        themeLabel: "UI Theme",
        themeDefault: "Default Theme",
        themePokemon: "Pokemon GO Theme",
        themeIos26: "Liquid Glass Theme",
        themeDark: "Dark Theme",
        customColorsTitle: "Custom Wayspot Colors",
        btnResetColors: "Restore Defaults",
        accMapThemes: "🗺️ Map & Themes",
        accWayspotStyles: "🎨 Wayspot Appearance",
        accControlTools: "🛠️ Control Tools",
        accSyncData: "☁️ Wayspot Data Management",
        accAboutApp: "🌐 Language Selection",
        credits: "Made by CA: Community Ambassador Thailand",
        btnDone: "Done",
        searchPlaceholder: "Search Wayspot name...",
        accProjects: "📂 Projects Management",
        btnNewProject: "➕ Create New Project",
        defaultProjectName: "Default Project",
        projectNameLabel: "Project Name",
        newProjectPrompt: "Please enter new project name:",
        renameProjectPrompt: "Rename project:",
        deleteProjectConfirm: "Are you sure you want to delete project '{name}'? (All data in this project will be deleted)",
        duplicateSuffix: " (Copy)",
        backupOptionsTitle: "Select Backup Option",
        backupActiveOnly: "Backup Active Project Only",
        backupAllProjects: "Backup All Projects",
        editTitle: "Edit Wayspot",
        editTypeLabel: "Type",
        btnCancel: "Cancel",
        btnSave: "Save",
        btnNext: "Next",
        btnBack: "Back",
        btnUnderstand: "Got it",
        menuTutorial: "📘 Getting Started",
        tutorialTitle: "Getting Started",
        tutorialSlide1Title: "📍 Adding Wayspots",
        tutorialSlide1Content: "Double click on map to add Wayspot or enter coordinates directly for precision!",
        tutorialSlide2Title: "✏️ Editing & Undo",
        tutorialSlide2Content: "Switch to Edit mode to drag markers, and use the Undo (↩️) button or Cmd+Z/Ctrl+Z to revert mistakes.",
        tutorialSlide3Title: "🌐 Planning Tools",
        tutorialSlide3Content: "Enable S2 Cells (L14/L17) and 45m Radius to verify spawn points and spacing.",
        tutorialSlide4Title: "🎨 Appearance",
        tutorialSlide4Content: "Switch to Dark Mode for eye comfort, and choose your preferred Map Layer (Google/OSM).",
        tutorialSlide5Title: "☁️ Cloud & Data",
        tutorialSlide5Content: "Sync with Google Drive or Import/Export JSON files to keep your data safe and portable.",
        tutorialSlide6Title: "📥 Wayfarer Import",
        tutorialSlide6Content: "Download the CA Exporter UserScript inside Settings to directly extract Wayspots to your map with one click.",
        tutorialSlide7Title: "🤝 Realtime Collaboration",
        tutorialSlide7Content: "Generate a session ID to share with your teammates, enabling everyone to pin and edit Wayspots together in real-time.",
        welcomeTitle: "Sawasdee! 👋",
        welcomeText: "Welcome to <b>CA Wayspot Tools</b>!<br><br>Easily simulate, measure, and plan CA Wayspots. Built in support for S2 Cells and KML files from Google My Maps.<br><br>Click anywhere on the map to place your first pin.",
        btnStart: "Get Started",
        noDataToSave: "No data to save",
        saveSuccess: "Successfully saved to Google Drive! ☁️",
        saveFailed: "Failed to save. Error: ",
        backupNotFound: "No backup file found in your Google Drive.",
        loadSuccess: "Data loaded successfully! ☁️",
        loadFailed: "Failed to restore: ",
        importSuccessCount: "Successfully imported {count} pins\n(Skipped {dup} duplicates)",
        noLocationFound: "Location data not found",
        invalidLatLong: "Please enter valid Latitude and Longitude.",
        deleteConfirmMark: "Delete this Wayspot?",
        invalidRadius: "Please enter a valid radius number.",
        confirmRadiusChange: "Are you sure you want to change the radius of all Wayspots to {radius} meters?",
        radiusChangeSuccess: "Successfully changed radius for {count} Wayspots!",
        clearAllConfirm: "Are you sure you want to clear all data?",
        unnamedAlert: "Unnamed",
        countLabel: "Total:",
        infoCoords: "Coords:",
        btnEdit: "Edit",
        btnLock: "🔒 Lock",
        btnUnlock: "🔓 Unlock",
        btnHideSpot: "👁 Hide",
        btnShowSpot: "🙈 Show",
        btnDelete: "Delete",
        editWayspot: "Edit Wayspot",
        errorLocation: "Cannot access your location. Please try again later.",
        importError: "An error occurred while loading the data. Please check the file format.",
        invalidName: "Please enter Wayspot name",
        invalidImageUrl: "Invalid Image URL format",
        accCollab: "🤝 Live Collaboration (Share Screen)",
        collabHostBtn: "Create Room (Host)",
        collabJoinBtn: "Join Room",
        collabIdPlaceholder: "Enter Room ID...",
        collabStatusPrefix: "Status: ",
        collabDisconnected: "Disconnected",
        collabConnectedHost: "Room {id}",
        collabConnectedClient: "Room {id}",
        collabDisconnectBtn: "Disconnect",
        collabSyncBtn: "Force Sync Data",
        collabSyncing: "Syncing... ({count} spots)",
        collabWait: "Waiting for peers on ID: {id}",
        collabConnectedHostRes: "Connected to peer!",
        collabConnectedClientRes: "Joined room {id} successfully!",
        collabConnectedClient: "Joined room {id} successfully!",
        collabError: "Connection failed: ",
        collabDisconnectBtn: "Disconnect",
        importWayfarerTitle: "Import Wayspot from Niantic Wayfarer",
        btnDownloadScript: "Download Script",
        btnInstallInstructions: "Install Instructions",
        installInstructionsList: "1. Click \"Download Script\"<br>2. Install the script via Tampermonkey or Userscripts<br>3. Log in to Niantic Wayfarer and go to Mapview page<br>4. Click \"Export CA\" and the system will download a file named CA_Wayfarer_500m_XXXX.json<br>5. Import the CA_Wayfarer_500m_XXXX.json file via \"Import JSON\"<br>6. Enjoy 🥰",
        dragDropText: "Drop file here to import data",
        accLivePoi: "📡 POI from Wayfarer",
        livePoiPowerSpotLabel: "Power Spot",
        livePoiTokenSectionTitle: "Campfire Session Token",
        livePoiTokenPlaceholder: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...",
        btnClear: "Clear",
        livePoiLoading: "Loading POI...",
        livePoiZoomInHint: "Zoom in further to see POI",
        livePoiTokenExpired: "Token expired or invalid — please get a new one",
        livePoiFetchError: "Failed to load POI data",
        livePoiNetworkError: "Connection failed — check your internet",
        livePoiReadOnlyNote: "Data from Wayfarer (read-only)",

        livePoiTokenCardTitle: "Campfire API Token",
        livePoiTokenNotConfigured: "Not set up yet — needed to see Power Spots",
        livePoiTokenExpiredShort: "⚠ Token expired — please update it",
        livePoiTokenReady: "Ready",
        livePoiTokenUpdatedAt: "Updated",
        livePoiTokenBtnSet: "Set up Token",
        livePoiTokenBtnUpdate: "Update Token",
        livePoiHowToComputer: "💻 Easy way — Computer",
        livePoiHowToComputerStep1: "Drag this button to your Bookmark bar:",
        livePoiHowToComputerStep2: "Open campfire.nianticlabs.com and log in with your own account",
        livePoiHowToComputerStep3: "Click the button you dragged to the Bookmark bar",
        livePoiHowToComputerStep4: "Come back here and click \"Paste from Clipboard\" below",
        livePoiHowToMobile: "📱 Easy way — iPhone/iPad (Safari)",
        livePoiHowToMobileStep1: "Tap this link once to install the Shortcut:",
        livePoiHowToMobileStep1b: "(or scan the QR code)",
        livePoiHowToMobileShortcutLink: "Add \"Campfire Token\" Shortcut",
        livePoiHowToMobileStep2: "Open campfire.nianticlabs.com in Safari and log in with your own account",
        livePoiHowToMobileStep3: "Tap Share → choose the \"Campfire Token\" Shortcut (tap Allow the first time)",
        livePoiHowToMobileStep4: "Come back here and tap \"Paste from Clipboard\" below",
        livePoiPasteClipboard: "📋 Paste from Clipboard",
        livePoiBearerLabel: "Bearer Token *",
        livePoiCsrfLabel: "X-Csrf-Token (optional)",
        livePoiHintText: "💡 If the two methods above don't work — open campfire.nianticlabs.com → DevTools → Network → click a request to niantic-social-api.nianticlabs.com/graphql → check Request Headers → Authorization (drop the \"Bearer \" prefix)",
        livePoiPrivacyNote: "🔒 Your Campfire API Token is stored on this device only — it is never uploaded or sent to anyone else.",
        hideItemTooltip: "Hide/show this point",
        hideAllTooltip: "Hide/show all in this category",
        selectAllTooltip: "Select all/none in this category",
        livePoiFromWayfarerSuffix: "from Wayfarer",
        drawModeHint: "Click the map to draw an area — double-click to finish",
        drawSelectionTitle: "Selected Area",
        drawNoSelection: "No Wayspots inside the drawn area",
        btnClearSelection: "Clear Selection",
        btnDeleteSelected: "Delete Selected",
        btnExportSelectedJson: "📄 Export JSON",
        btnExportSelectedKml: "📤 Export KML",
        confirmDeleteSelected: "Delete {count} selected points?\n(Your own Wayspots will be deleted for real; Wayfarer POIs will just be hidden from view)",
        livePoiPasteError: "Couldn't read the clipboard — try copying and pasting it into the Bearer Token field below yourself",
        livePoiBearerRequired: "Please enter a Bearer Token",
        livePoiSaving: "Verifying...",
        livePoiTokenInvalid: "Token doesn't work — check it was copied correctly and hasn't expired",
        wayfarerImportTitle: "📡 Wayfarer Wayspots Found",
        wayfarerImportDesc: "This file contains {count} Wayspot(s) marked as coming from Wayfarer. How would you like to import it?",
        wayfarerImportAll: "Import All Data",
        wayfarerImportUserOnly: "Import Only My Own Wayspots",
        wayfarerImportCancelled: "Import cancelled"
    }
};
